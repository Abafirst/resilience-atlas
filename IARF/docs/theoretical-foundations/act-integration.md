---
title: "ACT Integration in IATLAS"
category: theoretical-foundations
status: draft
last_updated: 2026-04-25
---

# Acceptance & Commitment Therapy (ACT) Integration in IATLAS

> *ACT doesn't ask you to fix yourself. It asks you to live — fully, flexibly, and in the direction of what matters most.*

---

## Overview

Acceptance and Commitment Therapy (ACT) is a contextual behavioral therapy grounded in Relational Frame Theory (RFT) and developed by Steven C. Hayes and colleagues. ACT targets **psychological flexibility** — the capacity to be fully present with your experience and take valued action, even in the face of difficult thoughts, feelings, and circumstances.

In IATLAS, ACT provides:
- The **values framework** that gives direction to resilience-building
- The **acceptance and defusion tools** that dissolve barriers to action
- The **present-moment orientation** that grounds intervention work
- The **contextual lens** that complements ABA's behavioral analysis

---

## Why ACT + ABA?

ABA and ACT share deep roots. Both emerge from behavioral science, both are evidence-based, and both focus on behavior change in context. Yet each brings something distinct:

| Dimension | ABA Contribution | ACT Contribution |
|-----------|-----------------|-----------------|
| **What we target** | Observable, measurable behavior | Private events (thoughts, feelings) as behavior |
| **How we understand behavior** | Antecedents, behaviors, consequences | Verbal behavior, relational framing, context |
| **What drives change** | Reinforcement and functional analysis | Values, acceptance, psychological flexibility |
| **Tools** | Shaping, chaining, reinforcement, data | Defusion, acceptance, values work, committed action |
| **Goal** | Measurable behavior change in context | A rich, full, meaningful life guided by values |

Together, they provide a complete framework: **ABA gives us the "how" of behavior change; ACT gives us the "why."**

---

## The ACT Hexaflex: Psychological Flexibility Model

ACT describes psychological flexibility through six interconnected processes, often depicted as the "Hexaflex":

```
         Present Moment Awareness
               /           \
    Acceptance               Defusion
          |     Psychological  |
    Self-as-Context   →   Values
               \           /
            Committed Action
```

### 1. Acceptance
*Willingness to have difficult thoughts and feelings without unnecessary struggle.*

**In resilience terms**: Accepting the pain of adversity rather than avoiding it is itself a resilient act. Experiential avoidance is one of the most common and costly barriers to resilience.

**IATLAS application**: Help clients develop willingness exercises; distinguish between acceptance (feeling the feeling) and resignation (giving up).

---

### 2. Defusion (Cognitive Defusion)
*Creating distance from thoughts — seeing thoughts as thoughts, not facts.*

**In resilience terms**: Unhelpful narratives ("I'll never recover," "I'm not strong enough") lose their power when clients can observe them without being fused to them.

**IATLAS application**: Defusion techniques (labeling thoughts, naming the story, using metaphor) are especially powerful in the Cognitive-Narrative dimension.

---

### 3. Present Moment Awareness
*Flexible, intentional contact with the present experience, as it is, right now.*

**In resilience terms**: Resilience is exercised *now* — not in a hypothetical future or a re-lived past. Present-moment awareness supports both somatic regulation and cognitive flexibility.

**IATLAS application**: Mindfulness practices, grounding exercises, sensory awareness activities.

---

### 4. Self-as-Context (The Observing Self)
*Experiencing oneself as the stable observer of one's experience, rather than as the content of that experience.*

**In resilience terms**: Identity fusion with adversity narratives ("I am broken," "I am a victim") is a barrier to resilience. Self-as-context provides a stable "self" that can hold even the most painful experiences without being defined by them.

**IATLAS application**: Observer self exercises, perspective-taking, narrative work in the Spiritual and Cognitive dimensions.

---

### 5. Values
*Chosen qualities of action — what matters most to this person, in this life.*

**In resilience terms**: Values provide direction and motivation for resilience-building. Knowing *why* you're doing the hard work makes the hard work possible.

**IATLAS application**: Values clarification exercises are foundational to every IATLAS intervention plan. Values guide goal-setting and connect daily practice to deeper meaning.

---

### 6. Committed Action
*Taking effective, values-guided action — even when it's difficult, uncertain, or uncomfortable.*

**In resilience terms**: Committed action is the behavioral expression of resilience. It's the observable, measurable end-product of the other five processes working together.

**IATLAS application**: Committed action planning bridges ACT values work with ABA behavioral goals and objectives. This is where the two approaches most directly converge.

---

## ACT Across the Six Dimensions

| Dimension | Primary ACT Processes |
|-----------|----------------------|
| **Agentic-Generative** | Values, Committed Action |
| **Relational-Connective** | Acceptance, Present Moment |
| **Spiritual-Reflective** | Values, Self-as-Context |
| **Emotional-Adaptive** | Acceptance, Defusion |
| **Somatic-Regulative** | Present Moment, Acceptance |
| **Cognitive-Narrative** | Defusion, Self-as-Context |

---

## ACT Metaphors & Experiential Exercises

ACT is notably metaphor-rich. Key metaphors used in IATLAS:

| Metaphor | ACT Process | Application |
|----------|-------------|-------------|
| **The Passenger on the Bus** | Defusion | Thoughts and feelings as passengers you don't have to obey |
| **Quicksand** | Acceptance | Struggling makes things worse; staying still is safe |
| **Leaves on a Stream** | Defusion | Watching thoughts float by without grabbing them |
| **The Compass** | Values | Values as direction, not destination |
| **Carrying Your Backpack** | Acceptance | The weight of our history — we carry it, but it doesn't define the journey |
| [PLACEHOLDER] | | Additional metaphors to be added per dimension |

---

## References

- Hayes, S. C., Strosahl, K. D., & Wilson, K. G. (2012). *Acceptance and commitment therapy: The process and practice of mindful change* (2nd ed.). Guilford Press.
- Hayes, S. C., Barnes-Holmes, D., & Roche, B. (Eds.). (2001). *Relational frame theory: A post-Skinnerian account of human language and cognition*. Kluwer Academic/Plenum Publishers.
- [PLACEHOLDER — add additional references as framework develops]

---

*See also: [ABA Principles](aba-principles.md) | [Six Dimensions](six-dimensions.md) | [Phase 3: Intervention](../methodology/03-intervention.md)*
