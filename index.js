...
app.post('/create-payment', verifyToken, async (req, res) => {
    try {
        ...
        const paymentIntent = await stripe.paymentIntents.create({
            ...
            metadata: { userId: req.userId, ...metadata }
        });
        ...
        res.status(200).json({ clientSecret: paymentIntent.client_secret, paymentIntentId: paymentIntent.id, amount: amount, currency: currency });
    } catch (error) {
        ...
        switch (event.type) {
            ...
            case 'payment_intent.payment_failed':
                ...
                await paymentsCollection.updateOne(
                    ...
                    { $set: { status: 'failed', failedAt: new Date(), failureReason: paymentIntentFailed.last_payment_error?.message } }
                );
            ...
            case 'charge.refunded':
                ...
                await paymentsCollection.updateOne(
                    ...
                    { $set: { status: 'refunded', refundedAt: new Date(), refundAmount: chargeRefunded.amount_refunded / 100 } }
                );
...
app.get('/payment/:paymentIntentId', verifyToken, async (req, res) => {
    try {
        ...
        const paymentsCollection = db.collection('payments');
        const payment = await paymentsCollection.findOne({ stripePaymentIntentId: paymentIntentId, userId: req.userId });
        if (!payment) {
